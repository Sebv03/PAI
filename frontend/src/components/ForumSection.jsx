...
const ForumSection = ({ courseId, isOwner, userRole }) => {
    ...
    const handleStartEdit = (announcement) => {
        ...
        setEditTitle(announcement.titulo || announcement.title);
        setEditContent(announcement.contenido || announcement.content);
    };
    ...
    return (
        <div className="card">
            <div className="flex justify-between items-center" style={{ marginBottom: '1.5rem' }}>
                ...
                {isOwner && userRole?.toLowerCase() === 'docente' && (
                    <button
            ...
            {showCreateForm && isOwner && userRole?.toLowerCase() === 'docente' && (
            ...
            {announcements.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    {announcements.map(announcement => {
                        ...
                        const canEdit = isOwner && userRole?.toLowerCase() === 'docente';
                        const canDelete = isOwner && userRole?.toLowerCase() === 'docente';
                                    ...
                                    <>
                                        <div className="flex justify-between items-start" style={{ marginBottom: '0.75rem' }}>
                                            <div style={{ flex: 1 }}>
                                                ...
                                                <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>
                                                    Por: {announcement.autor_nombre || announcement.author_name || `Usuario ${announcement.autor_id || announcement.author_id}`}
                                                </p>
                                                <p style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>
                                                    {new Date(announcement.fecha_creacion || announcement.created_at).toLocaleDateString('es-ES', {
                                                        year: 'numeric',
                                                        month: 'long',
                                                        day: 'numeric',
                                                        hour: '2-digit',
                                        ...
                                        <div style={{ marginBottom: '1rem', padding: '1rem', backgroundColor: 'var(--bg-tertiary)', borderRadius: 'var(--border-radius)' }}>
                                            <p style={{ fontSize: '0.875rem', color: 'var(--text-primary)', whiteSpace: 'pre-wrap', margin: 0 }}>
                                                {announcement.contenido || announcement.content}
                                            </p>
                                        </div>
                                                    ...
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                                        {announcementComments.map(comment => {
                                                            const canDeleteComment = user && (
                                                                user.id === (comment.autor_id || comment.author_id) ||
                                                                (isOwner && userRole?.toLowerCase() === 'docente') ||
                                                                userRole?.toLowerCase() === 'administrador'
                                                            );
                                                                    ...
                                                                    <div className="flex justify-between items-start">
                                                                        <div style={{ flex: 1 }}>
                                                                            <p style={{ fontSize: '0.8125rem', fontWeight: '600', marginBottom: '0.25rem', color: 'var(--text-primary)' }}>
                                                                                {comment.autor_nombre || comment.author_name || `Usuario ${comment.autor_id || comment.author_id}`}
                                                                            </p>
                                                                            <p style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', marginBottom: '0.5rem' }}>
                                                                                {new Date(comment.fecha_creacion || comment.created_at).toLocaleDateString('es-ES', {
                                                                                    year: 'numeric',
                                                                                    month: 'short',
                                                                                    day: 'numeric',
                                                                                    hour: '2-digit',
