...
const CourseDetail = () => {
    ...
    const fetchCourseDetails = async () => {
        ...
        try {
            ...
            if (user && (((user.rol || user.role)?.toLowerCase() === 'docente') || ((user.rol || user.role)?.toLowerCase() === 'administrador') || ((user.rol || user.role)?.toLowerCase() === 'estudiante'))) {
                try {
                    ...
                    if (((user.rol || user.role)?.toLowerCase() === 'estudiante') && taskErr.response?.status === 403) {
    ...
    return (
        <div className="container" style={{ paddingTop: '2rem', paddingBottom: '2rem' }}>
            ...
            <div className="card mb-xl">
                <h1 style={{ fontSize: '2rem', marginBottom: '0.5rem', color: 'var(--primary)' }}>{course.titulo || course.title}</h1>
                <p style={{ fontSize: '1.125rem', color: 'var(--text-secondary)', marginBottom: '0.75rem' }}>{course.descripcion || course.description}</p>
                <div style={{ fontSize: '0.875rem', color: 'var(--text-tertiary)', marginBottom: '0.5rem' }}>
                    ...
                    {(course.fecha_creacion || course.created_at) && (
                        <p style={{ marginBottom: '0.25rem' }}>
                            <strong style={{ color: 'var(--text-primary)' }}>Creado el:</strong> {new Date(course.fecha_creacion || course.created_at).toLocaleDateString('es-ES', {
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric'
                            })}
            ...
            <div className="course-detail-layout" style={{ 
                ...
                <div>
                    ...
                    <div>
                        <div className="flex justify-between items-center" style={{ marginBottom: '1rem' }}>
                            ...
                            {(user?.rol || user?.role) === 'estudiante' && tasks.length > 0 && (
                                <button
                                    onClick={() => {
                ...
                {isOwner && (user.rol || user.role) === 'docente' && (
