        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
            {courses.map((course) => (
                <div key={course.id} style={{ border: '1px solid #666', borderRadius: '8px', padding: '15px', backgroundColor: '#555', boxShadow: '0 2px 5px rgba(0,0,0,0.2)' }}>
                    <h4 style={{ margin: '0 0 10px 0', color: '#007bff' }}>{course.titulo || course.title}</h4>
                    <p style={{ fontSize: '0.9em', color: '#ccc', marginBottom: '10px' }}>{course.descripcion || course.description}</p>
