...
class ModelService:
    ...
    def train_model(self, features_df):
        ...
        classification_report_str = classification_report(
            y_test, y_pred, 
            target_names=['Riesgo Bajo', 'Riesgo Alto'],
            output_dict=False
        )
        
        # Generar reporte como diccionario para análisis programático
        ...
        # Agregar información detallada a las métricas
        metrics['classification_report'] = classification_report_str
        metrics['classification_report_dict'] = classification_report_dict
        metrics['confusion_matrix'] = cm.tolist()
        metrics['test_samples'] = len(y_test)
        metrics['train_samples'] = len(y_train)
        metrics['class_distribution'] = {
            'test': {
                'riesgo_bajo': int(np.sum(y_test == 0)),
                'riesgo_alto': int(np.sum(y_test == 1))
            },
            'train': {
                'riesgo_bajo': int(np.sum(y_train == 0)),
                'riesgo_alto': int(np.sum(y_train == 1))
            }
        }
