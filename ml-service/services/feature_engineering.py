...
class FeatureEngineering:
    ...
    def calculate_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Convertir fechas a datetime si son strings y normalizar timezone
        ...
        for col in date_columns:
            if col in data.columns:
                ...
                # Asegurar que todas tengan timezone UTC
    ...
    def _calculate_student_course_features(
        ...
        if not group.empty and 'submitted_at' in group.columns and 'due_date' in group.columns:
            ...
            if not valid_submissions.empty:
                # Normalizar fechas: convertir todas a UTC, manejando fechas con y sin timezone
                def normalize_datetime(series):
                    """Normaliza una serie de fechas a UTC"""
                    if series.empty:
                        return series
                    ...
                    if not pd.api.types.is_datetime64_any_dtype(series):
                        ...
                        # Si tiene timezone, convertir a UTC; si no, asumir UTC
                ...
                submitted_at_normalized = normalize_datetime(valid_submissions['submitted_at'])
                due_date_normalized = normalize_datetime(valid_submissions['due_date'])
                
                # Calcular delays en días
                delays = (submitted_at_normalized - due_date_normalized).dt.total_seconds() / 86400.0
