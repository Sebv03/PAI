...
def create_enrollment(db: Session, *, student_id: int, course_id: int) -> Enrollment:
    ...
    db_enrollment = Enrollment(
        ...
        curso_id=course_id
        # 'fecha_inscripcion' será generado automáticamente por el server_default=text("NOW()")
    )
...
def get_enrolled_courses_by_student(db: Session, student_id: int) -> List[Course]:
    ...
    # y filtramos donde el Enrollment.estudiante_id coincida.
    return db.query(Course).join(Enrollment).filter(Enrollment.estudiante_id == student_id).all()
...
def get_students_enrolled_in_course(db: Session, course_id: int) -> List[User]:
    ...
    # y filtramos donde el Enrollment.curso_id coincida.
    return db.query(User).join(Enrollment).filter(Enrollment.curso_id == course_id).all()
