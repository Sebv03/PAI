...
def get_courses_by_owner(db: Session, owner_id: int) -> List[Course]:
    ...
    """
    return db.query(Course).filter(Course.propietario_id == owner_id).all()
...
def create_user_course(db: Session, course_in: CourseCreate, owner: User) -> Course:
    ...
    """
    db_course = Course(
        titulo=course_in.titulo,
        descripcion=course_in.descripcion,
        propietario_id=owner.id # Asocia el curso al ID del propietario
    )
    db.add(db_course)
    db.commit()
...
def get_available_courses_for_student(db: Session, student_id: int, skip: int = 0, limit: int = 100) -> List[Course]:
    ...
    enrolled_enrollments = db.query(Enrollment.curso_id).filter(
