...
from datetime import datetime, timezone # Para validación de fechas
import os
import shutil
...
async def create_new_task(
    ...
    if task_in.fecha_limite < datetime.now():
        raise HTTPException(
...
async def read_task_by_id(
    ...
    if current_user.id == course.propietario_id: # Docente propietario del curso
        return task
...
async def read_tasks_by_course(
    ...
    if current_user.id == course.propietario_id: # Docente propietario
        tasks = crud_task.get_tasks_by_course(db, course_id=course_id, skip=skip, limit=limit)
        return tasks
...
async def update_existing_task(
    ...
    course = crud_course.get_course_by_id(db, course_id=db_task.curso_id)
    if course.propietario_id != current_user.id and current_user.rol != UserRole.ADMINISTRADOR:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    if task_in.fecha_limite and task_in.fecha_limite < datetime.now():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="La fecha de entrega no puede ser en el pasado."
        )
...
async def submit_task(
    ...
    if submitted_at is None:
        # Si por alguna razón la fecha no está disponible, usar la fecha actual
        submitted_at = datetime.now(timezone.utc)
        print(f"WARNING: fecha_entrega era None, usando fecha actual: {submitted_at}")
